...
def lambda_handler(event, context):
    try:
        ...
        folder = "uploads"
        file_name = f"{folder}/dni_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}_{random.randint(1000,9999)}.{ext}"
        ...
        return {
            'statusCode': 200,
            'headers': {'Access-Control-Allow-Origin': '*'},
            'body': json.dumps({'message': 'Image uploaded successfully', 'fileName': file_name})
        }
